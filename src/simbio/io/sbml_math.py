...
def root(*args):
    if len(args) != 2:
        raise TypeError(f"Expected only 2 arguments, but received {len(args)}")
...
mapper = {
    ...
    libsbml.AST_INTEGER: libsbml.ASTNode.getValue,
    libsbml.AST_REAL: libsbml.ASTNode.getValue,
    libsbml.AST_REAL_E: libsbml.ASTNode.getValue,
...
def replace(node: libsbml.ASTNode):
    ...
    if n == 0:
        ...
        children = [replace(node.getChild(n)) for n in range(node.getNumChildren())]
